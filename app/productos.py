...
# ✅ CORREGIDO: <int:id> en lugar de [int:id](int:id)
...
# ✅ CORREGIDO: <int:id> + DESCUENTO DE STOCK AL GENERAR REPORTE
...
def reporte(id):
    ...
    # Si es POST, se está confirmando el reporte y descontando stock
    if request.method == 'POST':
        # Obtener cantidad a reportar/vender (por defecto 1)
        ...
        if producto.stock >= cantidad:
            ...
            # Registrar la venta si se desea (opcional)
            # venta = Venta(total=producto.precio * cantidad, user_id=current_user.id)
            # db.session.add(venta)
            # db.session.flush()
            # detalle = VentaDetalle(venta_id=venta.id, producto_id=producto.id, cantidad=cantidad, precio_unitario=producto.precio)
    ...
    # Si es GET, solo mostrar el formulario de reporte
    return render_template('reporte_producto.html', producto=producto)
